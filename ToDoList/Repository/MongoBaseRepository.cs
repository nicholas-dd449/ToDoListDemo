...
namespace Repository
{
    public class MongoBaseRepository<T>: IMongoRepository<T> where T : class, new()
    {
        ...
        {
            var data = await _dbSet.FindAsync<T>(Builders<T>.Filter.Eq("_id", new ObjectId(id)));
        ...
        {
            var datas = await _dbSet.FindAsync(Builders<T>.Filter.Empty);
            return datas.ToList();
        }
        ...
        {
            return await Task.FromResult(_dbSet.AsQueryable<T>().Any(predicate));
        }
        ...
        public async Task<List<T>> FindListByPageAsync(FilterDefinition<T> filter, int pageIndex, int pageSize, string[]? field = null, SortDefinition<T>? sort = null)
        {
            ...
            if (sort == null)
                return await _dbSet.Find(filter).Project<T>(projection).Skip((pageIndex - 1) * pageSize).Limit(pageSize).ToListAsync();
